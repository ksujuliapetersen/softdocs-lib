	...
	if (boolEnable == true) {
		if (document.getElementById("softdocsLoadingBar")) {
		   $( '#softdocsLoadingBar' ).css('display', '');
			...
			$( 'body' ).prepend('<div id="softdocsLoadingBar" style="display: block; position: fixed; width: 80%; height: 220px; text-align: center; margin: 10%; z-index: 999; font-size: 26px; font-weight: bold; padding: 20px; background: white; box-shadow: 0 0 30px grey;"><div>Loading form, please wait...</div><div style="width: 100px; margin: 10px 0 0 calc(50% - 50px);"><object type="image/svg+xml" data="/dist/resources/images/svg/Loading.svg">...</object></div></div>');
		}
	} else {
